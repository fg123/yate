...
#define DECLARE_GROUP(name, from, to) \
  if (from <= id && id < to) return name;
...
ActionManager::ActionManager() {
  ...
  DECLARE_ACTION(
      30, "Save File", ctrl('s'),
      ACTION_FN { editor->getBuffer()->writeToFile(); });
  DECLARE_ACTION(
      40, "Revert File", NO_KEY, ACTION_FN { editor->revertBuffer(); });
  DECLARE_ACTION(
      50, "View Log", NO_KEY, ACTION_FN {
        editor->paneset_parent->replaceChildWith<LogPane>(
            editor->paneset_parent_child);
      });
  DECLARE_ACTION(
      60, "Actions / Keyboard Shortcuts", NO_KEY,
      ACTION_FN { yate.enterPrompt(new ActionsPromptWindow(yate)); });
  DECLARE_ACTION(
      70, "Configuration", NO_KEY,
      ACTION_FN { yate.enterPrompt(new ConfigPromptWindow(yate)); });
  DECLARE_ACTION(
      80, "Quit", ctrl('q'),
      ACTION_FN { yate.enterPrompt(new QuitPromptWindow(yate)); });

  DECLARE_ACTION(
      1000, "Undo", ctrl('z'), ACTION_FN {
        editor->getBuffer()->undo(editor->current_line, editor->current_col);
        editor->selection_start = NO_SELECTION;
      });
  DECLARE_ACTION(
      1010, "Redo", ctrl('y'), ACTION_FN {
        editor->getBuffer()->redo(editor->current_line, editor->current_col);
        editor->selection_start = NO_SELECTION;
      });
  DECLARE_ACTION(
      1020, "View History", ctrl('h'),
      ACTION_FN { yate.enterPrompt(new HistoryPromptWindow(yate, editor)); });
  DECLARE_ACTION(
      1030, "Copy", ctrl('c'), ACTION_FN {
  ...
  DECLARE_ACTION(
      1050, "Paste", ctrl('v'), ACTION_FN {
        if (yate.clipboard_buffers.size() > 0) {
          editor->paste(yate.clipboard_buffers.front());
        }
      });
  DECLARE_ACTION(
      1060, "Select All", ctrl('a'), ACTION_FN {
        editor->selection_start = LINECOL(0, 0);
        editor->current_line = editor->buffer->size() - 1;
        editor->current_col =
            editor->buffer->getLineLength(editor->current_line);
      });
  DECLARE_ACTION(
      1070, "Find", ctrl('f'),
      ACTION_FN { yate.enterPrompt(new FindPromptWindow(yate, editor)); });
  DECLARE_ACTION(
      1070, "Find Word", ctrl('j'), ACTION_FN {
        yate.enterPrompt(
            new FindPromptWindow(yate, editor, editor->getCurrentWord()));
      });
  DECLARE_ACTION(
      1080, "Find All", ctrl('d'),
      ACTION_FN { yate.enterPrompt(new FindAllPromptWindow(yate)); });
  DECLARE_ACTION(
      1081, "Previous Find", ctrl('r'), ACTION_FN {
        yate.enterPrompt(new FindPromptWindow(
            yate, editor, FindPromptWindow::Direction::Previous));
      });
  DECLARE_ACTION(
      1082, "Next Find", ctrl('t'), ACTION_FN {
        yate.enterPrompt(new FindPromptWindow(
            yate, editor, FindPromptWindow::Direction::Next));
      });

  DECLARE_ACTION(
      1090, "Choose Syntax", NO_KEY,
      ACTION_FN { yate.enterPrompt(new SyntaxPromptWindow(yate, editor)); });
  DECLARE_ACTION(
      1095, "Delete Word", 31, ACTION_FN { editor->deleteWord(); });
  DECLARE_ACTION(
      1096, "Delete Line", ctrl('l'), ACTION_FN { editor->deleteLine(); });
  DECLARE_ACTION(
      1100, "Indent", '\t', ACTION_FN {
        editor->getBuffer()->setRevisionLock();
        if (editor->selection_start == NO_SELECTION) {
          editor->insertTab(editor->current_line, editor->current_col);
        } else {
          LineNumber start =
              std::min(LINE(editor->selection_start), editor->current_line);
          LineNumber end =
              std::max(LINE(editor->selection_start), editor->current_line);
          ColNumber zero = 0;
  ...
  DECLARE_ACTION(
      1110, "Unindent", NO_KEY, ACTION_FN {
        editor->getBuffer()->setRevisionLock();
        if (editor->selection_start == NO_SELECTION) {
  ...
  DECLARE_ACTION(
      1999, "View Prefix Trie", ctrl('b'),
      ACTION_FN { yate.enterPrompt(new PrefixTrieWindow(yate, editor)); });

  DECLARE_ACTION(
      2000, "Choose Editor", ctrl('p'), ACTION_FN {
        yate.enterPrompt(new NavigateWindow(
            yate, (NavigateWindowProvider*)yate.getEditorNavigateProvider(),
            nullptr));
      });
  DECLARE_ACTION(
      2010, "Navigate by Tag", NO_KEY,
      ACTION_FN { yate.enterPrompt(new TagsPromptWindow(yate, editor)); });
  DECLARE_ACTION(
      2020, "Go to Line", ctrl('g'),
      ACTION_FN { yate.enterPrompt(new GoToLinePromptWindow(yate, editor)); });
  DECLARE_ACTION(
      2030, "Select Root", NO_KEY, ACTION_FN {
        yate.enterPrompt(new NavigateWindow(yate, yate.root, nullptr));
      });
  DECLARE_ACTION(
      2040, "Select Current Editor", NO_KEY, ACTION_FN {
  ...
  DECLARE_ACTION(
      3010, "Close Tab", ctrl('w'), ACTION_FN {
        TabSet* first = editor->findFirstParent<TabSet>();
        if (first) {
          first->closeTab();
        }
